...
const ProjectCard = ({ tech, index, showCardDetails, isItSelected, dimension }) => {
  ...
  // Instead of loading="lazy", handle image loading manually
  ...
  return (
    <>
      <div
        ...
        className={
          isItSelected
            ? 'items-center flex justify-center w-11/12 my-10 relative cursor-pointer '
            : 'cursor-pointer relative w-full flex flex-col justify-center relative'
        }
        key={index}
      >
        {onHover ? (
          <div
            className={
              onHover
                ? 'w-full lg:w-6/12 z-40 ease-in duration-600 bg-black/50 h-full absolute z-60 flex flex-col justify-center w-6/12 '
                : 'h-full absolute project-links z-60 flex flex-col justify-center'
            }
          >
            <div className="items-center flex justify-around">
              <a
                className="z-50 bg-green-700 hover:bg-green-500 rounded px-3 py-3 text-lg ease-in duration-300"
                href={tech.url}
                target="_blank"
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          {imageLoaded && (
            <img
              className={
                isItSelected
                  ? 'mx-auto w-full lg:w-6/12'
                  : 'w-full z-30 ease-in duration-300 lg:w-11/12'
              }
              src={tech.image}
              alt={tech.name}
              width={'50'}
